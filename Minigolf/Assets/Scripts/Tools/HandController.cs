...
public class HandController : MonoBehaviour
{
    ...
    private void HandlePutterHolding()
    {
        if(isLeftHand)
        {
            leftPutter.SetActive(true);
            rightPutter.SetActive(false);
        }

        if(isRightHand)
        {
            rightPutter.SetActive(true);
            leftPutter.SetActive(false);
        }
    }
